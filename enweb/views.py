from django.shortcuts import render,redirect
from website.models import *

def home(request):
    return render(request, 'en-pages/enhome.html', {})


def tyreHandler(request):
    tyrehandlers = TyreHandler.objects.all()
    count = len(tyrehandlers)
    return render(request, 'en-webproduct/enlistproduct.html', {'products': tyrehandlers, 'count': count})


def cylinderHandler(request):
    cylinderhandlers = CylinderHandler.objects.all()
    count = len(cylinderhandlers)
    return render(request, 'en-webproduct/enlistproduct.html', {'products': cylinderhandlers, 'count': count})


def strutHandler(request):
    struthandlers = StrutHandler.objects.all()
    count = len(struthandlers)
    return render(request, 'en-webproduct/enlistproduct.html', {'products': struthandlers, 'count': count})


def pipehandler(request):
    pipehandler = PipeHandler.objects.all()
    count = len(pipehandler)
    return render(request, 'en-webproduct/enlistproduct.html', {'products': pipehandler, 'count': count})


def wheelmotor(request):
    wheelmotor = Wheelmotor.objects.all()
    count = len(wheelmotor)
    return render(request, 'en-webproduct/enlistproduct.html', {'products': wheelmotor, 'count': count})


def belthandler(request):
    belthandler = BeltHandler.objects.all()
    count = len(belthandler)
    return render(request, 'en-webproduct/enlistproduct.html', {'products': belthandler, 'count': count})


def cablehandler(request):
    cablehandler = CableHandler.objects.all()
    count = len(cablehandler)
    return render(request, 'en-webproduct/enlistproduct.html', {'products': cablehandler, 'count': count})


def rodhandler(request):
    rodhandler = RodHandler.objects.all()
    count = len(rodhandler)
    return render(request, 'en-webproduct/enlistproduct.html', {'products': rodhandler, 'count': count})


def productcontrol(request, id, type):
    if type == 'cylinderhandler':
        return cylinderhandler_detail(request, id)
    elif type == 'struthandler':
        return struthandler_detail(request, id)
    elif type == 'tyrehandler':
        return tyrehandler_detail(request, id)
    elif type == 'belthandler':
        return belthandler_detail(request, id)
    elif type == 'wheelmotor':
        return wheelmotor_detail(request, id)
    elif type == 'pipehandler':
        return pipehandler_detail(request, id)
    elif type == 'rodhandler':
        return rodhandler_detail(request, id)
    elif type == 'cablehandler':
        return cablehandler_detail(request, id)


def tyrehandler_detail(request, id):
    tyrehandler = TyreHandler.objects.get(id=id)
    return render(request, 'en-webproduct/entyredetail.html', {'tyrehandler': tyrehandler})


def cylinderhandler_detail(request, id):
    cylinderhandler = CylinderHandler.objects.get(id=id)
    return render(request, 'en-webproduct/encylinderdetail.html', {'cylinderhandler': cylinderhandler})


def struthandler_detail(request, id):
    struthandler = StrutHandler.objects.get(id=id)
    return render(request, 'en-webproduct/enstrutdetail.html', {'struthandler': struthandler})


def belthandler_detail(request, id):
    belthandler = BeltHandler.objects.get(id=id)
    return render(request, 'en-webproduct/enbeltdetail.html', {'belthandler': belthandler})


def wheelmotor_detail(request, id):
    wheelmotor = Wheelmotor.objects.get(id=id)
    return render(request, 'en-webproduct/enwheeldetail.html', {'wheelmotor': wheelmotor})


def pipehandler_detail(request, id):
    pipehandler = PipeHandler.objects.get(id=id)
    return render(request, 'en-webproduct/enpipedetail.html', {'pipehandler': pipehandler})


def rodhandler_detail(request, id):
    rodhandler = RodHandler.objects.get(id=id)
    return render(request, 'en-webproduct/enrodhandler.html', {'rodhandler': rodhandler})


def cablehandler_detail(request, id):
    cablehandler = CableHandler.objects.get(id=id)
    return render(request, 'en-webproduct/encabledetail.html', {'cablehandler': cablehandler})


def contact(request):
    return render(request, 'en-pages/encontact.html', {})


def messages(request):
    name = request.POST['name']
    email = request.POST['email']
    subject = request.POST['subject']
    messages = request.POST['message']
    phone = request.POST['phone']
    message.objects.create(name=name, email=email, message=messages, title=subject, phone=phone)
    return redirect('enweb:home')


def about(request):
    return render(request, 'en-pages/enabout.html', {})


def history(request):
    posts = post.objects.all()
    return render(request, 'en-pages/enposts.html', {'posts': posts})


def surety(request):
    return render(request, 'en-pages/enguarantee.html', {})


def repair(request):
    return render(request, 'en-pages/enservice_repair.html', {})


def optimization(request):
    return render(request, 'en-pages/enoptimization.html', {})


def customization(request):
    return render(request, 'en-pages/encustomization.html', {})


def supplycomp(request):
    return render(request, 'en-pages/ensupply_comp.html', {})


def pluginsrender(request):
    Plugins = plugins.objects.all().reverse()
    return render(request, 'en-webproduct/enplugins.html', {'plugins': Plugins})
